#!/usr/bin/env python3
"""
Simple HTTP Server for PHP Simulation
Serves HTML files and simulates PHP backend functionality
"""
...
class RequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # Serve server.php (AJAX endpoint)
        elif path == '/server.php':
            stringa = query_params.get('stringa', [''])[0].lower()
            
            if len(stringa) > 0:
                # Filtra i comuni che iniziano con la stringa
                risultati = [c for c in COMUNI_DB if c['name'].lower().startswith(stringa)]
                
                # Ordina alfabeticamente
                risultati.sort(key=lambda x: x['name'])
                
                if risultati:
                    response = f"la select ha individuato {len(risultati)} alternative:<BR>\n"
                    response += "(comune, latitudine, longitudine)<BR><BR>\n"
                    for comune in risultati:
                        response += f"{comune['name']},{comune['lat']},{comune['lng']}<BR>\n"
                else:
...
if __name__ == '__main__':
    ...
    print("Apri il browser e visita: http://localhost:8000/client.html")
